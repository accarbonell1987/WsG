...
wsServer.on("request", function (request) {
  ...
  connection.on("message", function (message) {
    ...
    if (json_servidor.type === "auth_usuario") {
      ...
      Bd_cnx.usuarios_bd
        .find({ Usuario: json_servidor.data.Usuario }, function (err, bdUser) {
    ...
    if (json_servidor.type === "eliminar_usuario") {
      ...
      if (json_servidor.tipoUsuario === "0") {
        // caso usuario Admin.
        eError = 0;
      ...
      } else if (json_servidor.tipoUsuario === "1") {
        // caso usuario Doctor.
        var eError = 0;
        ...
        Bd_cnx.usuarios_bd
          .find({ Doctor_id: json_servidor.data }, "usuario_id", function (err, ePaciente) {
            if (err != null) {
              eError = 1;
          ...
          .then(() => {
            Bd_cnx.examen_bd
              .find({ $or: arrPaciente }, "examen_id", function (err, eExamenes) {
                if (err != null) {
                  eError = 1;
                  console.log("error: " + err);
                  return handleError(err);
              ...
              .then(() => {
                Bd_cnx.muestra
                  .find({ $or: arrExamenes }, "_id", function (err, eMuestras) {
                    if (err != null) {
                      eError = 1;
                      console.log("error: " + err);
                      return handleError(err);
                  ...
                  .then(() => {
                    Bd_cnx.muestra
                      .deleteMany({ $or: arrMuestras }, function (err) {
                      ...
                      .then(() => {
                        Bd_cnx.examen_bd
                          .deleteMany({ $or: arrExamenes }, function (err) {
                            //Eliminando examenes
                            if (err != null) {
                              eError = 1;
                              console.log("error: " + err);
                          ...
                          .then(() => {
                            Bd_cnx.usuarios_bd
                              .deleteMany({ $or: arrPaciente }, function (err) {
      ...
      } else if (json_servidor.tipoUsuario === "2") {
        // caso usuario Paciente.
        eUserTipo = 2;
        var eError = 0;
        Bd_cnx.examen_bd.find({ usuario_id: json_servidor.data }, "examen_id", function (err, eExamenes) {
          ...
          if (eExamenes.length > 0) {
            // si hay examenes para este usuario entonces proceder a eliminar sus muestras.
            var arArr = [];
            eExamenes.forEach((eElemento) => {
              // se eliminan todas las muestras de todos los examenes
              var tmp = {
            ...
            Bd_cnx.muestra
              .deleteMany({ $or: arArr }, function (err) {
                if (err != null) {
                  console.log("error: " + err);
                  eError = 1;
                }
              ...
              .then(
                Bd_cnx.examen_bd
                  .deleteMany({ usuario_id: json_servidor.data }, function (err) {
                    if (err != null) {
                      eError = 1;
                    }
                  })
                  .then(() =>
                    Bd_cnx.usuarios_bd.deleteMany({ usuario_id: json_servidor.data }, function (err) {
                      if (err != null) {
                        eError = 1;
                      }
                    })
            ...
            // en este caso no hay examenes por tanto solo queda eliminar el usuario.
            Bd_cnx.usuarios_bd.findOneAndDelete({ usuario_id: json_servidor.data }, function (err, user) {
              if (err != null) {
                eError = 1;
              }
    ...
    if (json_servidor.type === "lista_pacientes") {
      Bd_cnx.usuarios_bd
        .find({ Doctor_id: json_servidor.data }, function (err, Pacientes) {
          if (err) return handleError(err);
    ...
    if (json_servidor.type === "insertar_paciente") {
      ...
      // primero hay que validar los datos enviados.
      if (
        json_servidor.data.usuario_id === "" ||
        json_servidor.data.Nombre === "" ||
        json_servidor.data.Apellido === "" ||
        json_servidor.data.Correo === "" ||
        json_servidor.data.Edad === "" ||
        json_servidor.data.Genero === "" ||
        json_servidor.data.Estado_de_TX === "" ||
        json_servidor.data.Freq_Muestro === "" ||
        json_servidor.data.Periodo_de_Obs === "" ||
        json_servidor.data.Observaciones === ""
      ) {
        valCodigo = 1; // hay datos en blanco.
    ...
    if (json_servidor.type === "eliminar_pacientes") {
      // hay que eliminar las muestras los examnes y el paciente.
      var eError = 0;
      Bd_cnx.examen_bd.find({ usuario_id: json_servidor.data }, "examen_id", function (err, eExamenes) {
        if (err != null) {
          merror = 1;
    ...
    if (json_servidor.type === "lista_examenes") {
      Bd_cnx.examen_bd
        .find({ usuario_id: json_servidor.data }, function (err, Examenes) {
          if (err) return handleError(err);
    ...
    if (json_servidor.type === "eliminar_examen") {
      ...
      // 2. Borrar el examen .
      Bd_cnx.muestra
        .deleteMany({ examen_id: json_servidor.data }, function (err) {
          if (err != null) {
            eExamen = 1;
          }
        })
    ...
    if (json_servidor.type === "Enviar_BD") {
      ...
      Bd_cnx.muestra
        .find({ examen_id: json_servidor.examen_id }, "muestra est_tiempo")
        .skip(json_servidor.tbloque * json_servidor.pos)
        .limit(json_servidor.tbloque)
        .then((mmuestra) => {
          if (mmuestra.length > 0) {
            let datos_info = new Array(),
              datos_bd = new Array(),
              datos_rr = new Array();
            // si hay disponibilidad de datos entonces enviar.

            // inicializando ventana
            var tamVentana = Math.round((15 * (json_servidor.tbloque * 2)) / 100);

            for (var vv = 0; vv < tamVentana; vv++) {
            ...
            for (var f = 0; f < mmuestra.length; f++) {
              ...
              if (tmp_val_senal < rrUmbral) {
                if (val_ant === 1 && tmpAhora === 0) {
                  ...
                  if (total_muestras - r_ant > 0.1 * rr_medio) {
                    ...
                    // Fin Calculo RR.
                    rr_info = {
                      //Creo Objeto con las caracteristicas de la informacion RR Solicitada.
                      lat: latidos,
                      rr: tmp_rr,
              ...
              datos_bd.push({ time: total_muestras, valor: fPBYn });
              datos_rr.push({ time: total_muestras, valor: rr_valor });

              total_muestras++;
            }

            var msgjson = JSON.stringify({ type: "msg_bd", data: datos_bd, data2: datos_rr, rr_info: datos_info, fin_tx: false });
    ...
    if (json_servidor.type === "examen_id") {
      ...
      Bd_cnx.usuarios_bd
        .findOne({ usuario_id: idCliente }, "Freq_Muestro Periodo_de_Obs Estado_de_TX", function (err, dUsuario) {
          if (err) return handleError(err);
        ...
        .then(() => {
          ...
          for (var vv = 0; vv < tamVentana_TR; vv++) {
            //Inicializacion de la ventana
            ventana_TR.push(0);
          }
    ...
    if (json_servidor.type === "muestras") {
      ...
      if (idCliente > 0 && examen_id > 0 && tmpFM != 0) {
        json_servidor.data.forEach((muestra) => {
          ...
          var rt_obj = {
            //Creacion de la muestra en TR
            time: contTR,
            valor: /*Number(json_servidor.data)*/ fPBYn_TR,
          };

          var rr_rt_obj = {
            //Creacion de la serie RR en TR.
            time: contTR,
          ...
          if (cont_m_rt < tmpFM / 2 - 1 && contTR < tmpPObs * 60 * tmpFM - 1) {
            ...
            for (var crr_TR = 0; crr_TR < data_t_real_rr.length; crr_TR++) {
              ...
              if (tmp_val_senal_TR > rrUmbral) {
                ...
                if (tmp_val_senal_TR > max_ventana) {
                  // Hay un nuevo Maximo
  ...
  connection.on("close", function (connection) {
    ...
    console.log(new Date() + " Usuario : " + id_cnx_cliente + " desconectado.");
